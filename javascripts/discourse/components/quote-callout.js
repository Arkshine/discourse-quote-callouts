...
export default class QuoteCallout extends Component {
  ...
  constructor() {
    ...
    this.calloutSetting = findCalloutSetting(
      this.args.data.calloutSettings,
      this.args.data.type
    );

    // Canonical type (first type in callout setting)
    this.canonicalType =
    ...
    this.type =
      (this.calloutSetting ? this.args.data.type : null) ??
      this.canonicalType;

    this.title =
      nullIfEmpty(this.args.data.title) ??
    ...
    this.isCollapsable =
      nullIfEmpty(this.args.data.fold) &&
      this.args.data.children.length > 0;

    this.isCollapsed = this.isCollapsable && this.args.data.fold === "-";

    // Serialize children nodes into HTML string
    this.childrenHtml = this.args.data.children
